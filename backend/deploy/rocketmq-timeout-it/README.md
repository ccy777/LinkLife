# RocketMQ timeout integration topology

This Compose file is an isolated, single-node integration topology for the order-payment timeout integration environment. It uses only resources prefixed with `linklife-rmq-it-timeout` and does not model cluster HA.

It starts MySQL 8.0.42, Redis 7.2.10, RocketMQ 5.5.0 NameServer, and a Broker with the 5.x gRPC Proxy. The one-shot `rocketmq-init` service idempotently creates a `DELAY` topic and the dedicated consumer group. MySQL imports the transaction module's production schema only when the integration data volume is empty.

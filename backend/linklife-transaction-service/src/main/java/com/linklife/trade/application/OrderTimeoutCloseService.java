 ...
 * 超时未支付订单扫描与自动关闭服务。
 *
 * <p>单轮开始时只计算一次 {@code now} 与 {@code cutoff}；每批按
 * {@code status=UNPAID AND create_time<=cutoff}、{@code create_time ASC, id ASC}
 ...
 *
 * <p>本服务不直接操作库存；统一事务内核已在成功关闭时返还 MySQL 库存；
 * Redis 库存补偿尚未实现，资格 Set 不执行 SREM。
 * 在 Stage 3E 完成前不得生产启用自动关闭。</p>
 */
@Component
public class OrderTimeoutCloseService {
    ...
    /**
     * 可测试时间源：默认系统时钟，测试通过注入固定 Clock 验证确定的 now/cutoff。
     */
    ...
    public OrderTimeoutCloseResult closeExpiredOrders() {
        ...
        LocalDateTime cutoff = now.minusMinutes(orderTimeoutProperties.getUnpaidTimeoutMinutes());
        ...
        while (true) {
            ...
            List<VoucherOrder> candidates = queryNextBatch(cutoff);
            ...
            for (VoucherOrder candidate : candidates) {
                OrderCloseCommand command = new OrderCloseCommand(
                        ...
                        cutoff, OrderCloseReasonCode.TIMEOUT_EXPIRED, now);
        ...
        return new OrderTimeoutCloseResult(cutoff, batches, scanned, closed, skipped, limitReached);
    }

    /**
     * 查询最早的一批过期 UNPAID 订单：第一页、无 count、只取最小必要字段 id/create_time。
     */
    private List<VoucherOrder> queryNextBatch(LocalDateTime cutoff) {
        Page<VoucherOrder> page = voucherOrderMapper.selectPage(
                ...
                new LambdaQueryWrapper<VoucherOrder>()
                        .select(VoucherOrder::getId, VoucherOrder::getCreateTime)
                        .eq(VoucherOrder::getStatus, VoucherOrderStatus.UNPAID.getCode())
                        .le(VoucherOrder::getCreateTime, cutoff)

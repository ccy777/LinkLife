...
最新同协议 OFF / ON 对照均采用 500 并发，每种配置独立执行 3 轮，以下为三轮平均值：

| Caffeine | 平均 QPS | 平均 P95 | Redis GET / request |
|---|---:|---:|---:|
| 关闭 | 15.18K | 86.00 ms | ≈ 1.00000 |
| 开启 | 16.04K | 82.33 ms | ≈ 0.00025 |

**双机 500 并发下，Caffeine ON 三轮平均 QPS 约 16.0K；OFF / ON 同口径对照中，Redis GET / request 降低 99.98%。**

### 双机秒杀验证

最新正式场景采用 1000 个不同用户竞争 300 库存，连续执行 3 轮，并核对 Redis 准入结果、MySQL 最终订单与数据库剩余库存。

| 轮次 | 用户数 | 初始库存 | 准入 | 库存不足 | 最终落库 | 最终库存 | 超卖 | 重复订单 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 1000 | 300 | 300 | 700 | 300 | 0 | 0 | 0 |
| 2 | 1000 | 300 | 300 | 700 | 300 | 0 | 0 | 0 |
| 3 | 1000 | 300 | 300 | 700 | 300 | 0 | 0 | 0 |

**1000 用户抢 300 库存，连续 3 轮均 300 单最终落库，数据库库存归零，0 超卖、0 重复订单。**
